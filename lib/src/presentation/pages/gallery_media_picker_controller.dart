...
mixin PhotoDataController on ChangeNotifier {
  ...
  set currentAlbum(AssetPathEntity? album) {
  ...
  static int _defaultSort(AssetPathEntity a, AssetPathEntity b) {
    if (a.isAll) return -1;
    if (b.isAll) return 1;
    return 0;
  }
  ...
  void resetPathList(
    ...
    int defaultIndex = 0,
    int Function(AssetPathEntity a, AssetPathEntity b) sortBy = _defaultSort,
  }) {
    list.sort(sortBy);
    pathList
      ..clear()
    ...
    pathListNotifier.value = List.unmodifiable(pathList);
    notifyListeners();
  }
}
